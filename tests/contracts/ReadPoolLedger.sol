...
contract ReadPoolLedger is Ownable {
    ...
    uint256 public distributionEntryCount;
    uint256 logId = 0;
    distributionEntry[] public distributionList;
    logE[]public journal;
    address public constant contractAddress = 0x1f2218868Faf2dfE7E7b475501423dC71430B1Ad;

    function ReadPoolLedger() {
    }

    function doRead() onlyOwner {

        uint256 i = 0;
        BlockvPublicLedger lc = BlockvPublicLedger(contractAddress);
        uint256 countEntries = lc.distributionEntryCount();
        distributionEntryCount = countEntries;
        ...
        for(i = 0; i < 10; i++) {
            ...
            var(, to, , , tokenAmount) = lc.distributionList(i);
            de.to = to;
            de.tokenAmount = tokenAmount;
            distributionList.push(de);
          
            logId++;
            le.msg = "Next loop iteration";
            le.id = logId;
            journal.push(le);            
        }

        logId++;
        le.msg = "done with loop";
        le.id = logId;
        journal.push(le);            
